...
public class Generator : MonoBehaviour
{
    ...
    //nueva parte que es others contiene techo y decoraciones cofres
    private List<ObjectList> ceilings;
    private List<ObjectList> decorations;
    private List<ObjectList> chests;
    private int chestRate;
    private bool generateChests;
    private bool generateCeiling;
    private bool generateDecorations;
    private int decorationRate;
    
    // Variables para paredes con luces y decoraciones
    private List<ObjectList> wallLights;
    private List<ObjectList> wallDecorations;
    ...
    private void GetGenerationSettings()
    {
        ...
        //nueva parte que es others contiene techo luces y decoraciones cofres
        generateCeiling = settings.generateCeiling;
        ceilings = settings.ceilings;
        if (!CheckListObject(ceilings))
            Debug.Log("Ceilings are null or nonexistent.");

        wallLights = settings.wallLights;
        if (!CheckListObject(wallLights))
            Debug.Log("Wall Lights are null or nonexistent.");

        generateDecorations = settings.generateDecorations;
        decorations = settings.decorations;
        if (!CheckListObject(decorations))
            Debug.Log("Decorations are null or nonexistent.");

        decorationRate = settings.decorationRate;
        if (decorationRate == 0 && decorations.Count > 0)
            Debug.Log("Rate of Decorations can not equal 0 for Decorations objects to be generated.");

        generateChests = settings.generateChests;
        chests = settings.chests;
        if (!CheckListObject(chests))
            Debug.Log("Chests are null or nonexistent.");

        chestRate = settings.ChestRate;
        if (chestRate == 0 && chests.Count > 0)
            Debug.Log("Rate of Chests can not equal 0 for Chests objects to be generated.");

        wallDecorations = settings.wallDecorations;
        if (!CheckListObject(wallDecorations))
            Debug.Log("Wall Decorations are null or nonexistent.");

        wallLightRate = settings.wallLightRate;
        if (wallLightRate == 0 && wallLights.Count > 0)
            Debug.Log("Rate of Wall Lights can not equal 0 for Wall Lights objects to be generated.");
    ...
    private void CreateCorridor(int corridor, bool calculateNext, bool isFirstTime, Transform target, Transform startPoint)
    {
        ...
        if (corridor == 0)
        {
            ...
            do
            {
                ...
                // Piso central
                corridorFloor = Instantiate(flooring[randomFloor].gameObject,
                    new Vector3(startPoint.position.x, 0, startPoint.position.z + additive),
                    Quaternion.identity);
                corridorFloor.name = "Floor (" + tempIntX + ", " + tempIntY + ")";
                corridorFloor.transform.SetParent(corridors[corridorNumber].transform);

                // Piso a la derecha (extra ancho)
                GameObject corridorFloorRight = Instantiate(flooring[randomFloor].gameObject,
                    new Vector3(startPoint.position.x + 1, 0, startPoint.position.z + additive),
                    Quaternion.identity);
                corridorFloorRight.name = "Floor (" + (tempIntX + 1) + ", " + tempIntY + ")";
                corridorFloorRight.transform.SetParent(corridors[corridorNumber].transform);

                // Registro en área - evitar pisos duplicados
                ...
                if (area[tempIntX + 1, tempIntY] != null)
                {
                    DestroyImmediate(corridorFloorRight);
                }
                else
                {
                    area[tempIntX + 1, tempIntY] = corridorFloorRight;
                }
        ...
        else if (corridor == 1)
        {
            ...
            do
            {
                ...
                // Piso central
                corridorFloor = Instantiate(flooring[randomFloor].gameObject,
                    new Vector3(startPoint.position.x + additive, 0, startPoint.position.z),
                    Quaternion.identity);
                corridorFloor.name = "Floor (" + tempIntX + ", " + tempIntY + ")";
                corridorFloor.transform.SetParent(corridors[corridorNumber].transform);

                // Piso arriba (extra ancho)
                GameObject corridorFloorUp = Instantiate(flooring[randomFloor].gameObject,
                    new Vector3(startPoint.position.x + additive, 0, startPoint.position.z + 1),
                    Quaternion.identity);
                corridorFloorUp.name = "Floor (" + tempIntX + ", " + (tempIntY + 1) + ")";
                corridorFloorUp.transform.SetParent(corridors[corridorNumber].transform);
                ...
                if (area[tempIntX, tempIntY + 1] != null)
                {
                    DestroyImmediate(corridorFloorUp);
                }
                else
                {
                    area[tempIntX, tempIntY + 1] = corridorFloorUp;
                }
    ...
    private void CreateWalls(int a, int b)
    {
        ...
        {
            int angle = 0;
            GameObject wallToInstantiate = GetWallWithLightsOrDecorations(a, b, 0); // 0 = left side

            float displacement = CheckWallSize(wallToInstantiate, ref isXBigger);
            displacement = displacement / 2;

            // All walls use the same rotation logic
            if (isXBigger) angle = 90;

            area[a - 1, b] = Instantiate(wallToInstantiate, new Vector3(a - 0.5f - displacement, wallToInstantiate.transform.position.y, b), Quaternion.Euler(new Vector3(0, angle, 0)));
            area[a - 1, b].name = wallToInstantiate.name + " 1 A";
            //Check if walls are needed on the other side in the same block

            if (CheckNullAndTag("Floor", true, area[a - 2, b]))
            {
                angle = 0;
                GameObject wallToInstantiate2 = GetWallWithLightsOrDecorations(a, b, 0); // 0 = left side

                displacement = CheckWallSize(wallToInstantiate2, ref isXBigger);
                displacement = displacement / 2;
                ...
                GameObject localObject = Instantiate(wallToInstantiate2, new Vector3(a - 1.5f + displacement, wallToInstantiate2.transform.position.y, b), Quaternion.Euler(new Vector3(0, angle, 0)));
                localObject.name = wallToInstantiate2.name + " 1 B";
        ...
        {
            int angle = -90;
            GameObject wallToInstantiate = GetWallWithLightsOrDecorations(a, b, 1); // 1 = down side
            ...
            area[a, b - 1] = Instantiate(wallToInstantiate, new Vector3(a, wallToInstantiate.transform.position.y, b - 0.5f - displacement), Quaternion.Euler(new Vector3(0, angle, 0)));
            area[a, b - 1].name = wallToInstantiate.name + " 2 A";

            //Check if walls are needed on the other side in the same block
            if (CheckNullAndTag("Floor", true, area[a, b - 2]))
            {
                angle = -90;
                GameObject wallToInstantiate2 = GetWallWithLightsOrDecorations(a, b, 1); // 1 = down side
                displacement = CheckWallSize(wallToInstantiate2, ref isXBigger);
                displacement = displacement / 2;
                ...
                //Create a corridor on the other side, but the same block itself
                GameObject localObject = Instantiate(wallToInstantiate2, new Vector3(a, wallToInstantiate2.transform.position.y, b - 1.5f + displacement), Quaternion.Euler(new Vector3(0, angle, 0)));
                localObject.name = wallToInstantiate2.name + " 2 B";
                //Set this object's parent to be the object created previously, so when it needs to get destroyed it will get destroyed
                //Since it can't be accessed through the array.
        ...
        {
            int angle = 90;
            GameObject wallToInstantiate = GetWallWithLightsOrDecorations(a, b, 2); // 2 = up side
            ...
            area[a, b + 1] = Instantiate(wallToInstantiate, new Vector3(a, wallToInstantiate.transform.position.y, b + 0.5f + displacement), Quaternion.Euler(new Vector3(0, angle, 0)));
            area[a, b + 1].name = wallToInstantiate.name + " 3 A";
            //Check if walls are needed on the other side in the same block
            if (CheckNullAndTag("Floor", true, area[a, b + 2]))
            {
                angle = 90;
                GameObject wallToInstantiate2 = GetWallWithLightsOrDecorations(a, b, 2); // 2 = up side
                displacement = CheckWallSize(wallToInstantiate2, ref isXBigger);
                displacement = displacement / 2;
                ...
                //Create a corridor on the other side, but the same block itself
                GameObject localObject = Instantiate(wallToInstantiate2, new Vector3(a, wallToInstantiate2.transform.position.y, b + 1.5f - displacement), Quaternion.Euler(new Vector3(0, angle, 0)));
                localObject.name = wallToInstantiate2.name + "3 B";
                //Set this object's parent to be the object created previously, so when it needs to get destroyed it will get destroyed
                //Since it can't be accessed through the array.
                localObject.transform.parent = area[a, b + 1].transform;
            }
        ...
        {
            int angle = 180;
            GameObject wallToInstantiate = GetWallWithLightsOrDecorations(a, b, 3); // 3 = right side
            ...
            area[a + 1, b] = Instantiate(wallToInstantiate, new Vector3(a + 0.5f + displacement, wallToInstantiate.transform.position.y, b), Quaternion.Euler(new Vector3(0, angle, 0)));
            area[a + 1, b].name = wallToInstantiate.name + " 4 A";

            //Check if walls are needed on the other side in the same block
            if (CheckNullAndTag("Floor", true, area[a + 2, b]))
            {
                angle = 180;
                GameObject wallToInstantiate2 = GetWallWithLightsOrDecorations(a, b, 3); // 3 = right side
                //Create a corridor on the other side, but the same block itself
                displacement = CheckWallSize(wallToInstantiate2, ref isXBigger);
                displacement = displacement / 2;
                ...
                GameObject localObject = Instantiate(wallToInstantiate2, new Vector3(a + 1.5f - displacement, wallToInstantiate2.transform.position.y, area[a + 1, b].transform.position.z), Quaternion.Euler(new Vector3(0, angle, 0)));
                localObject.name = wallToInstantiate2.name + "4 B";
                //Set this object's parent to be the object created previously, so when it needs to get destroyed it will get destroyed
                //Since it can't be accessed through the array.
                localObject.transform.parent = area[a + 1, b].transform;
            }
    ...
    private void GenerateDoors()
    {
        ...
        {
            Debug.Log($"Generating doors with {doors.Count} door types available");
            ...
            for (int a = 0; a < areaWidth; a++)
            {
                for (int b = 0; b < areaHeight; b++)
                {
                    if (CheckNullAndTag("Floor", true, area[a, b]))
                    {
                        // Check for horizontal doors (vertical walls)
                        if (CheckNullAndTag("Floor", false, area[a + 1, b], area[a - 1, b]))
                        {
                            // Check if there's a floor above
                            if (CheckNullAndTag("Floor", true, area[a, b + 1]))
                            {
                                // Check if there's a floor diagonally above
                                if (CheckNullAndTag("Floor", true, area[a + 1, b + 1]) || CheckNullAndTag("Floor", true, area[a - 1, b + 1]))
                                {
                                    // Check for additional floor tiles to confirm corridor connection
                                    if (CheckNullAndTag("Floor", true, area[a - 1, b + 2]) || CheckNullAndTag("Floor", true, area[a + 1, b + 2]))
                                    {
                                        ...
                                        Debug.Log($"Door created at position: {pos}");
                                    }
                                }
                            }
                            ...
                            if (CheckNullAndTag("Floor", true, area[a, b - 1]))
                            {
                                // Check if there's a floor diagonally below
                                if (CheckNullAndTag("Floor", true, area[a + 1, b - 1]) || CheckNullAndTag("Floor", true, area[a - 1, b - 1]))
                                {
                                    // Check for additional floor tiles to confirm corridor connection
                                    if (CheckNullAndTag("Floor", true, area[a - 1, b - 2]) || CheckNullAndTag("Floor", true, area[a + 1, b - 2]))
                        ...
                        // Check for vertical doors (horizontal walls)
                        else if (CheckNullAndTag("Floor", false, area[a, b - 1], area[a, b + 1]))
                        {
                            // Check if there's a floor to the right
                            if (CheckNullAndTag("Floor", true, area[a + 1, b]))
                            {
                                // Check if there's a floor diagonally to the right
                                if (CheckNullAndTag("Floor", true, area[a + 1, b - 1]) || CheckNullAndTag("Floor", true, area[a + 1, b + 1]))
                                {
                                    // Check for additional floor tiles to confirm corridor connection
                            ...
                            // Check if there's a floor to the left
                            if (CheckNullAndTag("Floor", true, area[a - 1, b]))
                            {
                                // Check if there's a floor diagonally to the left
                                if (CheckNullAndTag("Floor", true, area[a - 1, b - 1]) || CheckNullAndTag("Floor", true, area[a - 1, b + 1]))
                                {
                                    // Check for additional floor tiles to confirm corridor connection
                                    if (CheckNullAndTag("Floor", true, area[a - 2, b - 1]) || CheckNullAndTag("Floor", true, area[a - 2, b + 1]))
            ...
            Debug.Log($"Total doors created: {doorGameObjects.Count}");
        }
        else
        {
            Debug.Log("No doors available for generation");
        }
    }
    ...
    //Parte nueva creada por gabriel donde sae generan los techos y las luces de pared ademas de los cofres y decoraciones tanto en paredes como en el suelo
    private void GenerateCeilings()
    {
        if (generateCeiling && ceilings != null && ceilings.Count > 0)
        {
            // Generate ceilings for all floor cells in the area
            for (int a = 0; a < areaWidth; a++)
            {
                for (int b = 0; b < areaHeight; b++)
                {
                    if (area[a, b] != null && area[a, b].CompareTag("Floor"))
                    {
                        int randomCeiling = Random.Range(0, ceilings.Count);
                        Vector3 ceilingPos = new Vector3(a, ceilings[randomCeiling].gameObject.transform.position.y, b);
                        GameObject ceiling = Instantiate(ceilings[randomCeiling].gameObject, ceilingPos, Quaternion.identity, area[a, b].transform.parent);
                        ceiling.tag = "Ceiling";
                        ceiling.name = "Ceiling (" + a + ", " + b + ")";
                        
                        // Add to the room's ceiling list
                        Room room = area[a, b].transform.parent.GetComponent<Room>();
                        if (room != null)
                        {
                            room.Ceilings.Add(ceiling);
                        }
                    }
                }
            }
    ...
    {
        if (generateDecorations && decorations != null && decorations.Count > 0)
        {
            for (int i = 0; i < parentRooms.Length; i++)
            {
                // Only generate decorations in rooms, not corridors
                for (int j = 0; j < parentRooms[i].transform.childCount; j++)
                {
                    GameObject floorTile = parentRooms[i].transform.GetChild(j).gameObject;
                    if (floorTile.CompareTag("Floor"))
                    {
                        int randomChance = Random.Range(0, 101);
                        if (randomChance <= decorationRate)
                        {
                            int randomDecoration = Random.Range(0, decorations.Count);
                            Vector3 decorationPos = floorTile.transform.position;
                            decorationPos.y = decorations[randomDecoration].gameObject.transform.position.y;
                            GameObject decoration = Instantiate(decorations[randomDecoration].gameObject, decorationPos, Quaternion.identity, parentRooms[i].transform);
                            decoration.tag = "Decoration";
                            decoration.name = "Decoration (" + i + ", " + j + ")";
                            parentRooms[i].Decorations.Add(decoration);
                        }
                    }
                }
            }
    ...
    {
        if (generateChests && chests != null && chests.Count > 0)
        {
            for (int i = 0; i < parentRooms.Length; i++)
            {
                // Skip player spawn room and boss room
                if (parentRooms[i] == playerSpawnRoom || parentRooms[i] == bossSpawnRoom)
                    continue;

                int randomChance = Random.Range(0, 101);
                if (randomChance <= chestRate)
                {
                    // Find a suitable floor tile for the chest
                    List<GameObject> availableFloors = new List<GameObject>();
                    for (int j = 0; j < parentRooms[i].transform.childCount; j++)
                    {
                        GameObject floorTile = parentRooms[i].transform.GetChild(j).gameObject;
                        if (floorTile.CompareTag("Floor"))
                        {
                            // Check if this position doesn't already have a decoration or chest
                            bool hasDecoration = false;
                            for (int k = 0; k < parentRooms[i].transform.childCount; k++)
                            {
                                GameObject child = parentRooms[i].transform.GetChild(k).gameObject;
                                if (child.CompareTag("Decoration") || child.CompareTag("Chest"))
                                {
                                    if (Vector3.Distance(child.transform.position, floorTile.transform.position) < 0.5f)
                                    {
                                        hasDecoration = true;
                                        break;
                                    }
                    ...
                    {
                        int randomFloor = Random.Range(0, availableFloors.Count);
                        int randomChest = Random.Range(0, chests.Count);
                        Vector3 chestPos = availableFloors[randomFloor].transform.position;
                        chestPos.y = chests[randomChest].gameObject.transform.position.y;
                        GameObject chest = Instantiate(chests[randomChest].gameObject, chestPos, Quaternion.identity, parentRooms[i].transform);
                        chest.tag = "Chest";
                        chest.name = "Chest (" + i + ")";
                        parentRooms[i].Chests.Add(chest);
                    }
                }
            }
        }
    ...
    private GameObject GetWallWithLightsOrDecorations(int floorX, int floorY, int wallSide)
    {
        // First check for wall lights (can be used in rooms and corridors)
        if (wallLights != null && wallLights.Count > 0)
        {
            int randomChance = Random.Range(0, 101);
            if (randomChance <= wallLightRate)
            {
                int randomWallLight = Random.Range(0, wallLights.Count);
                return wallLights[randomWallLight].gameObject;
            }
        }

        // Then check for wall decorations (only in rooms, not corridors)
        if (wallDecorations != null && wallDecorations.Count > 0)
        {
            // Check if this wall is part of a room (not a corridor)
            if (IsWallPartOfRoom(floorX, floorY))
            {
                int randomChance = Random.Range(0, 101);
                if (randomChance <= wallDecorationRate)
                {
                    int randomWallDecoration = Random.Range(0, wallDecorations.Count);
                    return wallDecorations[randomWallDecoration].gameObject;
                }
            }
        }

        // Default to normal wall
        int randomWall = Random.Range(0, walls.Count);
        return walls[randomWall].gameObject;
    }

 

    private bool IsWallPartOfRoom(int floorX, int floorY)
    {
        // Check if the floor tile at the given coordinates belongs to a room
        if (area != null && floorX >= 0 && floorX < areaWidth && floorY >= 0 && floorY < areaHeight)
        {
            if (area[floorX, floorY] != null && area[floorX, floorY].CompareTag("Floor"))
            {
                // Check if this floor tile's parent is a room
                if (area[floorX, floorY].transform.parent != null && area[floorX, floorY].transform.parent.CompareTag("Room"))
                {
                    return true;
                }
            }
